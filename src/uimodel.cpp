...
void UiModel::Impl::Init()
{
  ...
  m_VimInsertEscape = StrUtil::ToWString(UiConfig::GetStr("vim_insert_escape"));
  m_VimInsertEscapeTimeoutMs = UiConfig::GetNum("vim_insert_escape_timeout_ms");
...
void UiModel::Impl::VimHistoryUp()
{
  AnyUserKeyInput();
...
void UiModel::Impl::VimHistoryDown()
{
  AnyUserKeyInput();
  if (GetEditMessageActive()) return;
...
void UiModel::Impl::VimKeyHandler(wint_t p_Key)
{
  ...
  if (m_VimInsertMode && (m_VimInsertEscapeTimeMs != 0))
  {
    const int64_t elapsedMs = TimeUtil::GetCurrentTimeMSec() - m_VimInsertEscapeTimeMs;
    ...
    const bool validEscape = (m_VimInsertEscape.size() == 2) && (m_VimInsertEscapeTimeoutMs > 0);
    const bool sameEntry = validEscape && (m_CurrentChat.first == m_VimInsertEscapeProfileId) &&
      (m_CurrentChat.second == m_VimInsertEscapeChatId) && (entryPos == m_VimInsertEscapePos) &&
      (entryPos > 0) && (entryStr.at(entryPos - 1) == m_VimInsertEscape.at(0));
    const bool matches = validEscape && (p_Key == static_cast<wint_t>(m_VimInsertEscape.at(1))) &&
      (elapsedMs <= m_VimInsertEscapeTimeoutMs) && sameEntry;
    m_VimInsertEscapeTimeMs = 0;
    if (matches)
    {
      entryStr.erase(--entryPos, 1);
  ...
  if ((p_Key == keyDown) || (p_Key == keyUp))
  {
    (p_Key == keyDown) ? VimHistoryDown() : VimHistoryUp();
    return;
  }
  ...
  if (m_VimInsertMode)
  {
    ...
    {
      EntryKeyHandler(p_Key);
      if ((m_VimInsertEscape.size() == 2) &&
          (p_Key == static_cast<wint_t>(m_VimInsertEscape.at(0))) &&
          (m_VimInsertEscapeTimeoutMs > 0))
      {
        m_VimInsertEscapeTimeMs = TimeUtil::GetCurrentTimeMSec();
        m_VimInsertEscapeProfileId = m_CurrentChat.first;
        m_VimInsertEscapeChatId = m_CurrentChat.second;
        m_VimInsertEscapePos = m_EntryPos[m_CurrentChat.first][m_CurrentChat.second];
      }
    }
    return;
  }
...
void UiModel::Impl::VimNormalKey(wint_t p_Key)
{
  ...
  // Horizontal arrow keys retain compose cursor movement.
  if ((p_Key == keyLeft) || (p_Key == keyRight))
  {
    m_VimPendingKey = 0;
    m_VimCount = 0;
    for (int i = 0; i < count; ++i)
  ...
  if (!m_VimVisual && ((p_Key == L'j') || (p_Key == L'k')))
  {
    m_VimPendingKey = 0;
    m_VimCount = 0;
    for (int i = 0; i < count; ++i)
    {
      (p_Key == L'j') ? OnKeyNextChat() : OnKeyPrevChat();
    }
    return;
  }
  ...
  if (!m_VimVisual && ((p_Key == L'J') || (p_Key == L'K')))
  {
    ...
    for (int i = 0; i < count; ++i)
    {
      (p_Key == L'J') ? VimHistoryDown() : VimHistoryUp();
    }
    return;
  }
...
bool UiModel::Impl::DescribeImage()
{
  ...
  static const std::string describeImageCommand = UiConfig::GetStr("describe_image_command");
  std::string cmd = describeImageCommand;
  std::string scriptPath;
  if (cmd.empty())
  {
    scriptPath = FileUtil::GetTempDir() + "/describe";
    FileUtil::WriteFile(scriptPath, std::string((const char*)DescribeScript, sizeof(DescribeScript)));
    cmd = "python3 '" + scriptPath + "' -i '%1'";
  }

  std::string str;
  StrUtil::ReplaceString(cmd, "%1", StrUtil::EscapeSingleQuote(filePath));
  const bool rv = RunCommand(cmd, &str);
  if (!scriptPath.empty()) FileUtil::RmFile(scriptPath);
  if (rv)
  {
    if (!str.empty() && str.back() == '\n') str.pop_back();
    ...
    auto imageIt = std::find(msgVec.begin(), msgVec.end(), imageMsgId);
    if (imageIt != msgVec.end())
    {
      m_MessageOffset[profileId][chatId] = std::distance(msgVec.begin(), imageIt);
    }
